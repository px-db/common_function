...
def scan_file_in_dir(directory, recursive=False)->list:
    """
...
def file_list(files:list, extensions:str)->list:
...
def list_dir(dir_path, recursive=False):
    directories = []
...
def path_join(list_path_file:list)->str:
    """
    Fungsi untuk menggabungkan path file
    Parameter :
        list_path_file : list of path file
    Return :
        str : path file
    """
...
def scan_file_with_extension(path, extension, recursive=False):
    """
    Scan file dengan ekstensi tertentu dalam direktori
    
    Args:
        path (str): Path direktori yang akan di-scan
        extension (str): Ekstensi file yang dicari (contoh: 'txt', 'csv')
        recursive (bool): Jika True, akan mencari secara rekursif di subdirektori
    
    Returns:
        list: Daftar file dengan ekstensi yang diminta
    """
    # Konversi ke Path object
    path = Path(path)
    ...
    # Periksa apakah direktori valid
    if not path.is_dir():
        raise ValueError("Path yang diberikan bukan direktori yang valid")
    
    if recursive:
        list_files = []
        for files in path.rglob(f'*.{extension}'):
        ...
        return [str(f) for f in path.iterdir() if f.is_file() and f.suffix.lower() == f'.{extension.lower()}']
...
if os.name == 'nt':  # Windows
    def map_network_drive(drive_letter, network_path, username=None, password=None):
        ...
        try:
            ...
            win32net.NetUseDel("", drive_letter, 2)  # 2 = USE_FORCE
        except Exception:
        ...
        try:
            ...
            use_info = {
                ...
                "password": password,
                "username": username,
                "asg_type": 0,  # 0 = USE_DISK
            }
            win32net.NetUseAdd("", 2, use_info)
...
def generate_credentials_file(file_path: Optional[str] = None):
    """
    Menghasilkan file credential dengan format "user=" dan "pass=".
    """
    if not file_path:
        file_path = str(Path.cwd() / 'credentail')
    else:
...
def get_sqlite_tables(database_sqlite)->list:
    """
...
def get_columns_from_table_sqlite(database_sqlite, table_name)->list:
    """
...
def excel_to_sqlite(file_path: str, db_name: str, table_name: str, sheet_name: Optional[str] = None) -> None:
    """
...
def copy_column_table(source_db: str, target_db: str, table_name: str) -> None:
    """
...
def csv_to_dictionary(file_csv):
    """
    Mengonversi file CSV ke dictionary.

    Args:
        file_csv (str): Path ke file CSV yang akan dikonversi.

    Returns:
        dict: Dictionary yang berisi data dari file CSV.
    """
    try:
        # Membaca file CSV
        ...
        print(f"Error: File '{file_csv}' kosong.")
        return {}
    except pd.errors.ParserError:
        print(f"Error: Terjadi kesalahan saat mem-parsing file '{file_csv}'.")
        return {}
    except Exception as e:
        print(f"Error tak terduga saat membaca file '{file_csv}': {e}")
        return {}
...
def move_file(source_file, destination_file):
    """
    Memindahkan file dari lokasi sumber ke lokasi tujuan menggunakan shutil.move().

    Args:
        source_file (str): Path ke file yang akan dipindahkan.
        destination_file (str): Path ke lokasi tujuan file.

    Returns:
        bool: True jika file berhasil dipindahkan, False jika gagal.
    """
    try:
        # Memindahkan file
        shutil.move(source_file, destination_file)
        print(f"File berhasil dipindahkan dari '{source_file}' ke '{destination_file}'.")
        return True
    except FileNotFoundError:
        ...
        print(f"Error tak terduga saat memindahkan file: {e}")
        return False

def move_file_with_copy_delete(source_file, destination_file):
    """
    Memindahkan file dengan cara copy lalu delete menggunakan shutil.copy2() dan os.remove().
    ...
    try:
        ...
        print(f"Error: Tidak memiliki izin untuk memindahkan file.")
        return False
    except Exception as e:
        print(f"Error tak terduga saat memindahkan file: {e}")
        return False

def move_file_with_pathlib(source_file, destination_file):
    """
    Memindahkan file menggunakan pathlib.Path.rename().
...
def move_file_with_os_rename(source_file, destination_file):
    """
    Memindahkan file menggunakan os.rename().
    ...
    try:
        ...
        print(f"Error sistem operasi saat memindahkan file: {e}")
        return False
    except Exception as e:
        print(f"Error tak terduga saat memindahkan file: {e}")
        return False

def move_file_safe(source_file, destination_file, overwrite=False):
    """
    Memindahkan file dengan pengecekan keamanan.

    Args:
        source_file (str): Path ke file yang akan dipindahkan.
        destination_file (str): Path ke lokasi tujuan file.
        overwrite (bool): Apakah boleh menimpa file yang sudah ada.
    ...
    try:
        ...
        if dest_path.exists() and not overwrite:
            print(f"Error: File tujuan '{destination_file}' sudah ada. Gunakan overwrite=True untuk menimpa.")
            return False
...
def get_filename_from_path(path_file):
    """
    Mengambil nama file dari path file.

    Args:
        path_file (str): Path ke file.
...
def query_sqlite_columns(database_sqlite, table_name, columns=None, where_clause=None, limit=None):
    """
    Query database SQLite untuk kolom tertentu.

    Args:
        database_sqlite (str): Nama database SQLite.
        table_name (str): Nama tabel.
        columns (list, optional): Daftar kolom yang ingin diambil. Jika None, ambil semua kolom.
        where_clause (str, optional): Kondisi WHERE untuk filter data.
        limit (int, optional): Batas jumlah baris yang diambil.
...
def query_sqlite_single_column(database_sqlite, table_name, column_name, where_clause=None, distinct=False):
    """
    Query database SQLite untuk satu kolom tertentu.

    Args:
        database_sqlite (str): Nama database SQLite.
        table_name (str): Nama tabel.
        column_name (str): Nama kolom yang ingin diambil.
        where_clause (str, optional): Kondisi WHERE untuk filter data.
        distinct (bool): Apakah menggunakan DISTINCT untuk menghilangkan duplikat.

    Returns:
        list: List nilai dari kolom yang diquery.
    """
    try:
        # Buat query SELECT
        if distinct:
        ...
        print(f"Error saat query kolom {column_name}: {e}")
        return []

def query_sqlite_count(database_sqlite, table_name, where_clause=None):
    """
    Query database SQLite untuk menghitung jumlah baris.
    ...
    Returns:
        int: Jumlah baris yang memenuhi kondisi.
    """
    try:
        # Buat query COUNT
        ...
        print(f"Error saat menghitung baris: {e}")
        return 0

def query_sqlite_aggregate(database_sqlite, table_name, column_name, aggregate_func, where_clause=None):
    """
    Query database SQLite untuk fungsi agregat (SUM, AVG, MAX, MIN, COUNT).

    Args:
        database_sqlite (str): Nama database SQLite.
        table_name (str): Nama tabel.
        column_name (str): Nama kolom untuk agregat.
        aggregate_func (str): Fungsi agregat (SUM, AVG, MAX, MIN, COUNT).
        where_clause (str, optional): Kondisi WHERE untuk filter data.

    Returns:
        float/int: Hasil fungsi agregat.
    """
    try:
        # Validasi fungsi agregat
        valid_functions = ['SUM', 'AVG', 'MAX', 'MIN', 'COUNT']
        if aggregate_func.upper() not in valid_functions:
            raise ValueError(f"Fungsi agregat harus salah satu dari: {valid_functions}")
        
        # Buat query agregat
        query = f"SELECT {aggregate_func.upper()}({column_name}) FROM {table_name}"
        ...
        print(f"Error saat query agregat {aggregate_func}({column_name}): {e}")
        return None

def query_sqlite_group_by(database_sqlite, table_name, group_columns, select_columns=None, where_clause=None, having_clause=None, order_by=None):
    """
    Query database SQLite dengan GROUP BY.

    Args:
        database_sqlite (str): Nama database SQLite.
        table_name (str): Nama tabel.
        group_columns (list): Daftar kolom untuk GROUP BY.
        select_columns (list, optional): Daftar kolom yang ingin diambil.
        where_clause (str, optional): Kondisi WHERE untuk filter data.
        having_clause (str, optional): Kondisi HAVING untuk filter hasil GROUP BY.
        order_by (str, optional): Pengurutan hasil.
    ...
    try:
        ...
        # Tambahkan HAVING clause jika ada
        if having_clause:
        ...
        print(f"Error saat query GROUP BY: {e}")
        return pd.DataFrame()

def query_sqlite_join(database_sqlite, table1, table2, join_condition, select_columns=None, join_type="INNER", where_clause=None):
    """
    Query database SQLite dengan JOIN.

    Args:
        database_sqlite (str): Nama database SQLite.
        table1 (str): Nama tabel pertama.
        table2 (str): Nama tabel kedua.
        join_condition (str): Kondisi JOIN (contoh: "table1.id = table2.id").
        select_columns (list, optional): Daftar kolom yang ingin diambil.
        join_type (str): Tipe JOIN (INNER, LEFT, RIGHT, FULL).
        where_clause (str, optional): Kondisi WHERE untuk filter data.
    ...
    try:
        ...
        if select_columns:
            columns_str = ', '.join(select_columns)
            query = f"SELECT {columns_str} FROM {table1}"
        else:
            query = f"SELECT * FROM {table1}"
        
        # Tambahkan JOIN
        query += f" {join_type.upper()} JOIN {table2} ON {join_condition}"
        ...
        print(f"Error saat query JOIN: {e}")
        return pd.DataFrame()

def query_sqlite_like(database_sqlite, table_name, column_name, pattern, where_clause=None):
    """
    Query database SQLite dengan LIKE untuk pencarian pattern.

    Args:
        database_sqlite (str): Nama database SQLite.
        table_name (str): Nama tabel.
        column_name (str): Nama kolom untuk pencarian.
        pattern (str): Pattern untuk pencarian (gunakan % untuk wildcard).
        where_clause (str, optional): Kondisi WHERE tambahan.

    Returns:
        pd.DataFrame: DataFrame yang berisi hasil query.
    """
    try:
        # Buat query dengan LIKE
        like_condition = f"{column_name} LIKE '{pattern}'"
        
        if where_clause:
            query = f"SELECT * FROM {table_name} WHERE {where_clause} AND {like_condition}"
        else:
            query = f"SELECT * FROM {table_name} WHERE {like_condition}"
        ...
        print(f"Error saat query LIKE: {e}")
        return pd.DataFrame()

def query_sqlite_between(database_sqlite, table_name, column_name, min_value, max_value, where_clause=None):
    """
    Query database SQLite dengan BETWEEN untuk range nilai.

    Args:
        database_sqlite (str): Nama database SQLite.
        table_name (str): Nama tabel.
        column_name (str): Nama kolom untuk range.
    ...
    """
    try:
        # Buat query dengan BETWEEN
        between_condition = f"{column_name} BETWEEN {min_value} AND {max_value}"
        
        if where_clause:
            query = f"SELECT * FROM {table_name} WHERE {where_clause} AND {between_condition}"
        else:
            query = f"SELECT * FROM {table_name} WHERE {between_condition}"
        ...
        print(f"Error saat query BETWEEN: {e}")
        return pd.DataFrame()

def query_sqlite_in(database_sqlite, table_name, column_name, values_list, where_clause=None):
    """
    Query database SQLite dengan IN untuk multiple values.

    Args:
        database_sqlite (str): Nama database SQLite.
        table_name (str): Nama tabel.
        column_name (str): Nama kolom untuk filter.
        values_list (list): Daftar nilai yang dicari.
        where_clause (str, optional): Kondisi WHERE tambahan.
    ...
    """
    try:
        # Buat string values untuk IN clause
        if isinstance(values_list[0], str):
            values_str = "', '".join(values_list)
            in_condition = f"{column_name} IN ('{values_str}')"
        else:
            values_str = ", ".join(map(str, values_list))
            in_condition = f"{column_name} IN ({values_str})"
        
        if where_clause:
            query = f"SELECT * FROM {table_name} WHERE {where_clause} AND {in_condition}"
        else:
            query = f"SELECT * FROM {table_name} WHERE {in_condition}"
        ...
        print(f"Error saat query IN: {e}")
        return pd.DataFrame()

def query_sqlite_subquery(database_sqlite, main_query, subquery, where_clause=None):
    """
    Query database SQLite dengan subquery.

    Args:
        database_sqlite (str): Nama database SQLite.
        main_query (str): Query utama.
        subquery (str): Subquery yang akan disisipkan.
        where_clause (str, optional): Kondisi WHERE tambahan.
    ...
    """
    try:
        # Gabungkan main query dengan subquery
        if where_clause:
            query = f"{main_query} WHERE {where_clause} AND ({subquery})"
        else:
            query = f"{main_query} WHERE ({subquery})"
        ...
        print(f"Error saat query dengan subquery: {e}")
        return pd.DataFrame()

def query_sqlite_union(database_sqlite, query1, query2, union_type="UNION"):
    """
    Query database SQLite dengan UNION.

    Args:
        database_sqlite (str): Nama database SQLite.
        query1 (str): Query pertama.
        query2 (str): Query kedua.
        union_type (str): Tipe UNION (UNION, UNION ALL).
    ...
    try:
        ...
        query = f"{query1} {union_type.upper()} {query2}"
        ...
        print(f"Error saat query UNION: {e}")
        return pd.DataFrame()
